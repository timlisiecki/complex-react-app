...
function HomeGuest() {
	...
	function ourReducer(draft, action) {
		switch (action.type) {
			...
			case 'usernameAfterDelay':
				if (draft.username.value.length < 6) {
					draft.username.hasErrors = true;
					draft.username.message = 'Username must be at least 6 characters long.';
				}
			...
			case 'emailUniqueResults':
				if (action.value) {
					...
					draft.email.message = 'That email address is already taken.';
				} else {
		...
		if (state.submitCount) {
			...
			async function fetchResults() {
				try {
					...
					appDispatch({ type: 'flashMessage', value: 'Welcome to ComplexApp!' });
				} catch (err) {
					console.log(err);
				}
			}
	...
	function handleSubmit(e) {
		...
		// Submit form after validations pass
